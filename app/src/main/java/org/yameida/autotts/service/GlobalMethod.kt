...
import org.yameida.autotts.MyApplication
    ...
    while (!find) {
        ...
        if (!find) {
            if (isAtHome()) {
                ...
                backPress()
                //如果在登录页面就提示关闭autotts主功能
                if (AccessibilityUtil.findOnceByText(getRoot(), "手机号登录", exact = true) != null) {
                    LogUtils.e("登录前请先关闭AutoTTS主功能！")
                    ToastUtils.show("登录前请先关闭AutoTTS主功能！")
 ...
 * 获取前台窗口
 */
fun getRoot(ignoreCheck: Boolean = true): AccessibilityNodeInfo {
    while (true) {
